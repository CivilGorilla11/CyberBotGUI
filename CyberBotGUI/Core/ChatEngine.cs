...
namespace CyberBotGUI.Core
{
    ...
    public class ChatEngine
    {
        ...
        public string ProcessInput(string userInput)
        {
            ...
            ActivityLog.Add("User Message", $"Input received: \"{userInput.Substring(0, Math.Min(40, userInput.Length))}...\"", "USER");
            ...
            switch (intent)
            {
                ...
                case Intent.CompleteTask:
                    ...
                    response = cId > 0
                        ...
                        : "Which task number would you like to complete? Say 'show tasks' to see all tasks.";
                    return Log(response);
                ...
                case Intent.DeleteTask:
                    ...
                    response = dId > 0
                        ...
                        : "Which task number would you like to delete? Say 'show tasks' to see all tasks.";
                    return Log(response);

                case Intent.SetReminder:
                    response = "To set a reminder say: add task [task title] at [time] and I will prompt you for a reminder.";
                    return Log(response);
                ...
                case Intent.ShowLog:
                    ActivityLog.Add("Log Viewed", "User requested to view the activity log", "SYSTEM");
                    response = ActivityLog.FormatForChat();
                    return Log(response);

                case Intent.Exit:
                    ActivityLog.Add("Session Ended", "User has closed the chat session", "SYSTEM");
            ...
            try
            {
                ...
                response = empathyPrefix + ResponseLibrary.GetRandom(ResponseLibrary.FallbackResponses) +
                    "\n\n Tip: Try saying 'show tasks' or 'start quiz', or ask me about cybersecurity topics like 'phishing', 'malware'.";
                return Log(response);
            }
            catch (Exception ex)
            {
        ...
        private string HandlePassword(string i) => ResponseLibrary.GetRandom(ResponseLibrary.PasswordResponses);
        private string HandlePhishing(string i) => ResponseLibrary.GetRandom(ResponseLibrary.PhishingResponses);
        private string HandleScam(string i) => ResponseLibrary.GetRandom(ResponseLibrary.ScamResponses);
        private string HandleMalware(string i) => ResponseLibrary.GetRandom(ResponseLibrary.MalwareResponses);
        private string HandleRansomware(string i) => ResponseLibrary.GetRandom(ResponseLibrary.RansomwareResponses);
        private string HandleFirewall(string i) => ResponseLibrary.GetRandom(ResponseLibrary.FirewallResponses);
        private string HandleVpn(string i) => ResponseLibrary.GetRandom(ResponseLibrary.VpnResponses);
        private string HandleHacker(string i) => ResponseLibrary.GetRandom(ResponseLibrary.HackerResponses);
        private string HandleEncryption(string i) => ResponseLibrary.GetRandom(ResponseLibrary.EncryptionResponses);
        private string HandleBackup(string i) => ResponseLibrary.GetRandom(ResponseLibrary.BackupResponses);
    }
}
